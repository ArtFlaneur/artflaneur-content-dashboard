...
const clearWorkspaceScopeButton = document.querySelector("#clearWorkspaceScope");
const workspaceScopeSummary = document.querySelector("#workspaceScopeSummary");
...
const deletePersonaModal = document.querySelector("#deletePersonaModal");
const deletePersonaTitle = document.querySelector("#deletePersonaTitle");
const deletePersonaMessage = document.querySelector("#deletePersonaMessage");
const deletePersonaReassignField = document.querySelector("#deletePersonaReassignField");
const deletePersonaReplacement = document.querySelector("#deletePersonaReplacement");
const closeDeletePersonaModalButton = document.querySelector("#closeDeletePersonaModal");
const cancelDeletePersonaButton = document.querySelector("#cancelDeletePersona");
const confirmDeletePersonaButton = document.querySelector("#confirmDeletePersona");
...
function getPersonaDependencies(personaName) {
  const clusters = dashboardData.clusters.filter((cluster) => cluster.persona === personaName);
  const contentItems = Object.values(dashboardData.pipeline)
    .flat()
    .filter((item) => item.persona === personaName);

  return { clusters, contentItems };
}

function migratePersonaReferences(oldName, newName) {
  if (!oldName || !newName || oldName === newName) {
    return { clusterCount: 0, contentCount: 0 };
  }

  let clusterCount = 0;
  let contentCount = 0;

  dashboardData.clusters = dashboardData.clusters.map((cluster) => {
    if (cluster.persona !== oldName) {
      return cluster;
    }

    clusterCount += 1;
    return { ...cluster, persona: newName };
  });

  Object.values(dashboardData.pipeline).forEach((lane) => {
    lane.forEach((item) => {
      if (item.persona === oldName) {
        item.persona = newName;
        contentCount += 1;
      }
    });
  });

  return { clusterCount, contentCount };
}

function describeWorkspaceScope() {
  if (selectedFilters.persona === "All personas" && selectedFilters.stage === "All stages") {
    return "Showing all personas across all buyer stages.";
  }

  if (selectedFilters.persona === "All personas") {
    return `Showing all personas for the ${selectedFilters.stage} stage.`;
  }

  if (selectedFilters.stage === "All stages") {
    return `Showing ${selectedFilters.persona} across all buyer stages.`;
  }

  return `Showing ${selectedFilters.persona} in the ${selectedFilters.stage} stage.`;
}

function applyWorkspaceScope(nextPersona = selectedFilters.persona, nextStage = selectedFilters.stage) {
  selectedFilters.persona = nextPersona;
  selectedFilters.stage = nextStage;
  renderAll();
}

function closeDeletePersonaModal() {
  deletePersonaModal.hidden = true;
  activeDeletePersonaId = null;
  deletePersonaReplacement.innerHTML = "";
}
...
function openDeletePersona(personaId) {
  const persona = dashboardData.personas.find((item) => item.id === personaId);

  if (!persona) return;

  const { clusters, contentItems } = getPersonaDependencies(persona.name);
  const replacementOptions = dashboardData.personas.filter((item) => item.id !== personaId);
  const hasDependencies = clusters.length > 0 || contentItems.length > 0;

  activeDeletePersonaId = personaId;
  deletePersonaTitle.textContent = `Delete ${persona.name}`;

  if (hasDependencies) {
    deletePersonaMessage.textContent = `${persona.name} is linked to ${pluralize(clusters.length, "cluster")} and ${pluralize(contentItems.length, "content item")}. Choose a replacement persona before deleting it.`;
    deletePersonaReassignField.hidden = false;
    deletePersonaReplacement.innerHTML = replacementOptions
      .map((item) => `<option value="${item.name}">${item.name}</option>`)
      .join("");
    confirmDeletePersonaButton.textContent = "Reassign and delete persona";
    confirmDeletePersonaButton.disabled = replacementOptions.length === 0;

    if (replacementOptions.length === 0) {
      deletePersonaMessage.textContent = `${persona.name} is still linked to saved clusters or content items, but there is no replacement persona available. Add or keep another persona before deleting this one.`;
    }
  } else {
    deletePersonaMessage.textContent = `${persona.name} has no linked clusters or content items. You can delete it directly.`;
    deletePersonaReassignField.hidden = true;
    confirmDeletePersonaButton.textContent = "Delete persona";
    confirmDeletePersonaButton.disabled = false;
  }

  deletePersonaModal.hidden = false;
}
...
function saveEditPersona(event) {
  ...
  const duplicate = dashboardData.personas.find(
    (persona) => persona.id !== activeEditPersonaId && normalizeLabel(persona.name) === normalizeLabel(newName)
  );

  if (duplicate) {
    setAiStatus("Persona names must stay unique", "error");
    return;
  }
  ...
  const migrated = migratePersonaReferences(oldName, newName);
  ...
  setAiStatus(
    migrated.clusterCount || migrated.contentCount
      ? `Persona updated and moved ${pluralize(migrated.clusterCount, "cluster")} plus ${pluralize(migrated.contentCount, "content item")}`
      : "Persona updated",
    "success"
  );
...
function confirmDeletePersona() {
  const persona = dashboardData.personas.find((item) => item.id === activeDeletePersonaId);

  if (!persona) {
    closeDeletePersonaModal();
    return;
  }

  const { clusters, contentItems } = getPersonaDependencies(persona.name);
  const hasDependencies = clusters.length > 0 || contentItems.length > 0;
  const replacementName = hasDependencies ? deletePersonaReplacement.value : "";

  if (hasDependencies && !replacementName) {
    setAiStatus("Choose a replacement persona before deleting this one", "error");
    return;
  }

  const migrated = hasDependencies
    ? migratePersonaReferences(persona.name, replacementName)
    : { clusterCount: 0, contentCount: 0 };

  dashboardData.personas = dashboardData.personas.filter((item) => item.id !== persona.id);

  if (selectedFilters.persona === persona.name) {
    selectedFilters.persona = replacementName || "All personas";
  }
  ...
  setAiStatus(
    hasDependencies
      ? `Persona deleted and moved ${pluralize(migrated.clusterCount, "cluster")} plus ${pluralize(migrated.contentCount, "content item")} to ${replacementName}`
      : "Persona deleted",
    "success"
  );
  showSection("personas");
...
function buildFilters() {
  ...
  if (!personaOptions.includes(selectedFilters.persona)) {
    selectedFilters.persona = "All personas";
  }

  if (!stageOptions.includes(selectedFilters.stage)) {
    selectedFilters.stage = "All stages";
  }
  ...
  if (workspaceScopeSummary) {
    workspaceScopeSummary.textContent = `${describeWorkspaceScope()} Reset the scope any time to return to a full dashboard view.`;
  }

  if (aiContextPersona) {
    aiContextPersona.textContent = selectedFilters.persona;
  }

  if (aiContextStage) {
    aiContextStage.textContent = selectedFilters.stage;
  }
}
            ...
            <article class="persona-card">
              <div class="card-head card-head-split">
                ...
                <div class="card-actions-pair">
                  <button class="ghost-button card-action" type="button" data-focus-persona="${persona.name}">
                    ${selectedFilters.persona === persona.name ? "Focused" : "Focus"}
                  </button>
                  ${selectedFilters.persona === persona.name ? '<button class="ghost-button card-action" type="button" data-reset-persona="true">Show all</button>' : ""}
...
personaFilter.addEventListener("change", (event) => {
  applyWorkspaceScope(event.target.value, selectedFilters.stage);
});

stageFilter.addEventListener("change", (event) => {
  applyWorkspaceScope(selectedFilters.persona, event.target.value);
});

clearWorkspaceScopeButton?.addEventListener("click", () => {
  applyWorkspaceScope("All personas", "All stages");
  setAiStatus("Workspace scope reset", "success");
});
...
personaGrid.addEventListener("click", (event) => {
  const focusTrigger = event.target.closest("[data-focus-persona]");
  if (focusTrigger) {
    applyWorkspaceScope(focusTrigger.dataset.focusPersona, selectedFilters.stage);
    showSection("personas");
    return;
  }

  const resetTrigger = event.target.closest("[data-reset-persona]");
  if (resetTrigger) {
    applyWorkspaceScope("All personas", selectedFilters.stage);
    showSection("personas");
    return;
  }
  ...
  if (deleteTrigger) {
    openDeletePersona(deleteTrigger.dataset.deletePersona);
  }
});
...
closeDeletePersonaModalButton.addEventListener("click", closeDeletePersonaModal);
cancelDeletePersonaButton.addEventListener("click", closeDeletePersonaModal);
deletePersonaModal.addEventListener("click", (event) => {
  if (event.target === deletePersonaModal) {
    closeDeletePersonaModal();
  }
});
confirmDeletePersonaButton.addEventListener("click", confirmDeletePersona);
